...
export default function CartView(){
    ...
    useEffect(()=>{
        axios.get(`${process.env.REACT_APP_API_URL}/cart/viewCart`,
            {
                headers:{
    ...
    function deleteCartHandler(id){
        // 
        axios.delete(`${process.env.REACT_APP_API_URL}cart/deleteProduct/${id}`,
            {
                headers:{
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }  
...
function createOrder(event, quantityLocal,props){
    ...
    const allCart = cart.map((item, index)=> <CartCard key={index} {...item} deleteCartHandler={deleteCartHandler} createOrder={createOrder}/>)

    return(
        <>
            <h4 className="pt-5 ps-2 text-secondary">{cart.length ? "Your Cart" : "You don't have products on your cart"}</h4>
